...
import {
    companyStore,
    createAddCompanyAction,
    createDeleteCompanyAction,
    createFetchCompaniesAction,
...
class AdminService{
    ...
    public async addCustomer(customer:CustomerModel){
        ...
        customerStore.dispatch(addCustomerAction(newCustomer));
    }
    ...
    public async getAllCustomers(){
        if(customerStore.getState().customers.length === 0){
            ...
            customerStore.dispatch(fetchCustomersAction(customers))
            return customers
    ...
    public async updateCustomer(customer:CustomerModel){
        ...
        customerStore.dispatch(editCustomerAction(newCustomer))
        return newCustomer
    }

    public async deleteCompany(id:number){
        companyStore.dispatch(createDeleteCompanyAction(id))
        return (await axios.delete<string>(appConfig.adminUrl + "company/" + id)).data
    }

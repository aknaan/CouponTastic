...
class CustomerService{
    ...
    public async getCoupons(){
        if(couponStore.getState().coupons.length === 0){
            couponStore.dispatch(fetchCouponsAction((await axios.get<CouponModel[]>(appConfig.customerUrl + "coupons")).data))
        }
        return couponStore.getState().coupons;
    }

    //Didn't use the filter calls because they don't allow for cross-filtering
